  ...
  build(aTonicName, aStartingGridY) {
    if (Array.isArray(this.mode)) {
      ...
      for (let i = 1; i <= this.mode.length; i++) {
        ...
        let l_noteName = findNoteFromInterval(
          this.notes[i - 1].name,
          2,
          this.mode[i - 1]
        )
        ...
        for (let i = 0; i < this.notes.length; i++) {
          if (this.notes[i].isBeingMoved) {
            ...
            let isNotOnAnotherNote = this.notes.every(function (aNote) {
              return aNote.name !== thisScale.notes[i].name
                ? aNote.yGrid !== thisScale.notes[i].yGrid
                : aNote.yGrid !== thisScale.notes[i].pYGrid
            })
            if (
              isNotOnAnotherNote &&
              this.notes[i].yGrid > this.notes[0].yGrid
            ) {
              l_newY = true
            } else {
            ...
            if (l_newY) {
              ...
              for (let j = 0; j < this.notes.length; j++) {
                if (
                  this.notes[j].yGrid < this.notes[i].yGrid &&
                  this.notes[j].yGrid > l_previousNoteInTheScale.yGrid
                ) {
                  l_previousNoteInTheScale = this.notes[j]
                }
              }
              let l_semitone =
                this.notes[i].yGrid - l_previousNoteInTheScale.yGrid
              this.notes[i].name = findNoteFromInterval(
                l_previousNoteInTheScale.name,
                2,
                l_semitone
              )
              this.notes[i].frequency = this.parseFrequencyFromName(
                this.notes[i].name,
                false,
                this.notes[i].yGrid
              )
