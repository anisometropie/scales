...
const FRENCH_LANGUAGE_MAP = new Map( [      ["C", "do"],
                                            ["D", "ré"],
                                            ["E", "mi"],
                                            ["F", "fa"],
                                            ["G", "sol"],
...
const ALTERATION_REGEX = new RegExp("^\u266f$|^#$|^\u266d$|^b$|^\u{1d12a}$|^##$|^\u{1d12b}$|^bb$");
...
function setup() {
    ...
    for (let l_noteIndex=0; l_noteIndex<naturalScale.length; l_noteIndex++) {
        for (let l_offset = -2; l_offset <= 2; l_offset++) {
            try {
                let l_note = findNoteFromInterval(naturalScale[0], l_noteIndex+1, l_semitoneOnNaturalNotes + l_offset);
